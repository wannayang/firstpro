	...
	var jsonMyPagination = $("#pageBar").myPagination(
		{
			...
			cssStyle : 'jogger',//分页样式，具体样式可以从css/page.css样式中选取   jogger
				ajax : {
					...
					url : 'customer/getQueryZsJsonList?'+vals,   //?idnumber=2&name=比
					dataType : 'json',
						...
						$("#customerTable> tbody").html(""); //每次加载的时候先设为空
							$.each(data.event,function(i, value) {
								$("#customerTable> tbody").append("<tr>"
										...
										+ "<a href=\"javascript:void(0)\" onclick=\"delcustomer('" + value.id.cph + "','"+value.id.tbnf+"');\" class=\"btn btn-danger btn-xs active\" role=\"button\">删除</a>"
										+ "</td>"										
										+"<td> "+ value.id.cph + "</td>"
										+"<td> "+ value.id.tbnf + "</td>"
										+"<td> "+ value.xbnx + "</td>"
...
function cxmxAddIndex(cph,tbnf){
	...
	$("#queryForm").attr("action", 'customer/cxmxAddIndex').submit();
}
    ...
    <a  class="current">员工车险信息管理</a>
		...
		<div class="container-fluid">
          <div class="widget-title"> <span class="icon"> <i class="icon-th"></i> </span>
            <h5>员工车险正式信息管理</h5>
            <span class="label label-info"><a href="javascript:void(0);" onclick="exportfile();" class ="btn btn-info btn-xs">导出</a></span>
		</div>
               ...
               <div class="col-sm-12">
                   ...
                   <div class="form-group">
                       <input type="text" class="input-sm form-control" id="czxm" name="czxm" placeholder="车主姓名">
				   </div>
                   <div class="form-group">
                       <input type="text" class="input-sm form-control" id="czsfzh" name="czsfzh" placeholder="车主身份证号">
				   </div>
                   <div class="form-group">
                       <input type="text" class="input-sm form-control" id="cjh" name="cjh" placeholder="车架号">
				   </div>
                   <div class="form-group">
                       <input type="text" class="input-sm form-control" id="fdjh" name="fdjh" placeholder="发动机号">
				   </div>				   
				</div>
				<div class="col-sm-12">
					<div class="form-group">
						<label class=" control-label">单位</label> <select name="dw" id="dw" type="text" class="form-control"></select>
					</div>
					<div class="form-group">	
						<label class=" control-label">级别</label> <select name="jb" id="jb" type="text" class="form-control" ></select>
					</div>
					<div class="form-group">
						<label class=" control-label">承保公司</label> <select name="cbgs" id="cbgs" type="text" class="form-control" ></select>
					</div>
					<div class="form-group">
						<label class=" control-label">承保渠道</label> <select name="cbqd" id="cbqd" type="text" class="form-control" required></select>
					</div>
                   <div class="form-group">
                       <input type="text" class="input-sm form-control" id="jbrxm" name="jbrxm" placeholder="经办人">
				   </div>					
				</div>				
				
				<div class="col-sm-12">
					<div class="form-group">
						<label class=" control-label">起保日期起</label> <input class="input-sm datepicker-input-yyyy-mm-dd form-control" type="text" id="qbrq_start" name="qbrq_start" placeholder="请选择日期" />
					</div>
						~
					<div class="form-group">	
						<label class=" control-label">起保日期止</label> <input class="input-sm datepicker-input-yyyy-mm-dd form-control" type="text" id="qbrq_end" name="qbrq_end" placeholder="请选择日期" />
					</div>
					<div class="form-group">
						<label class=" control-label">终保日期起</label> <input class="input-sm datepicker-input-yyyy-mm-dd form-control" type="text" id="zbrq_start" name="zbrq_start" placeholder="请选择日期" />
					</div>
						~
					<div class="form-group">
						<label class=" control-label">终保日期止</label> <input class="input-sm datepicker-input-yyyy-mm-dd form-control" type="text" id="zbrq_end" name="zbrq_end" placeholder="请选择日期" />
					</div>
					</div>
					<div class="col-sm-12">
					<div class="form-group">
						<label class=" control-label">初登日期起</label> <input class="input-sm datepicker-input-yyyy-mm-dd form-control" type="text" id="cdrq_start" name="cdrq_start" placeholder="请选择日期" />
					</div>
						~
					<div class="form-group">
						<label class=" control-label">初登日期止</label> <input class="input-sm datepicker-input-yyyy-mm-dd form-control" type="text" id="cdrq_end" name="cdrq_end" placeholder="请选择日期" />
					</div>					
					<div class="form-group">
						<label class=" control-label">驾驶证到证日期起</label> <input class="input-sm datepicker-input-yyyy-mm-dd form-control" type="text" id="jszdzrq_start" name="jszdzrq_start" placeholder="请选择日期" />
					</div>
						~
					<div class="form-group">
						<label class=" control-label">驾驶证到证日期止</label> <input class="input-sm datepicker-input-yyyy-mm-dd form-control" type="text" id="jszdzrq_end" name="jszdzrq_end" placeholder="请选择日期" />
					</div>
					</div>
					<div class="col-sm-12">
					<div class="form-group">
						<label class=" control-label">缴费日期起</label> <input class="input-sm datepicker-input-yyyy-mm-dd form-control" type="text" id="jfrq_start" name="jfrq_start" placeholder="请选择日期" />
					</div>
						~
					<div class="form-group">
						<label class=" control-label">缴费日期止</label> <input class="input-sm datepicker-input-yyyy-mm-dd form-control" type="text" id="jfrq_end" name="jfrq_end" placeholder="请选择日期" />
					</div>					
					</div>
					<div class="form-group">					
					<a href="javascript:void(0);" onclick="queryck();" class ="btn btn-sm btn-info">查询</a>
				<!-- 	<a href="javascript:void(0);" onclick="btnclear();" class ="btn btn-sm btn-info">清空</a> -->
                	</div>
                </div>
               </form>
