		...
		function init(param,obj){
			...
			var defaults = {
              ...
              panel: {
                ...
                links: '#',
                tipInfo_on: true,
                tipInfo: '<span>&nbsp;&nbsp;跳{currText}/{sumPage}页&nbsp;&nbsp;共{rowCount}条</span>',
                tipInfo_css: {
                  width: '50px'
                }
              }
			...
			function getParam() {
				var param = "page="+opts.currPage;	//默认发送
				 for ( var p in opts.ajax.param ){
					 param += "&" +p+"="+ opts.ajax.param[p];
				 }
//				if (opts.ajax.param!='') {
...
//				console.log('param:'+param);
			...
			function getPanelTipInfo() {
				...
				if(opts.panel.tipInfo_on){
                	...
                	info = info.replace("{rowCount}", opts.rowCount);
			...
			function responseHandle(data){
                ...
                case "json":
					try{
						...
						rowCount = eval("data.pageBean.totalRow");
					}
                  break;
                case "xml":
